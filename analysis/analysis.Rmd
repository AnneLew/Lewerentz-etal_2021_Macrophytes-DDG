...
Chem_overview_SmallD<-cbind(Chem_overview_SmallDatasetMorph,Chem_overview_SmallDataset[,2:40])
Chem_overview_SmallD
# 
# Chem<-unname(Chem_overview_SmallDataset)
# 
# lak=Chem[,2:4]
# for (i in 2:13){
#   lak <- rbind(lak,Chem[,((i*3)-1):(1+(i*3))])
# }
# i=2
# Chem[,((i*3)-1):(1+i*3)]
# lak
...
```{r, code_folding: hide, fig.height=3, fig.width=8}
...
```{r Peak plots, echo=FALSE, fig.height=3, fig.width=8}
...
PPbeta<-ggplot(PEAK, aes(x=BetaPeakDepth, y=BetaPeakRichness, col=datasettotsimpl))+
  geom_point()+ 
  scale_x_reverse()+
...
PPgamma<-ggplot(PEAK, aes(x=GammaPeakDepth, y=GammaPeakRichness, col=datasettotsimpl))+
  geom_point()+ 
  scale_x_reverse()+
...
PEAK_Chem_norm[c(33,20,22,31,32,29,30,16,19,3:15)] %>% correlate() %>% network_plot(min_cor=0.5)
```
...
```{r PCA, echo=FALSE,fig.height=3, fig.width=8}
################ PCA ###############################################
library(ggbiplot)
...
```{r GAMM, echo=FALSE,fig.height=3, fig.width=8}
library(gamm4)
# gam_AlphaPeakDepth <- gamm4((AlphaPeakDepth) ~ s(Morph_PC1)+s(Morph_PC3),
...
# gam_AlphaPeakRichness <- gamm4(AlphaPeakRichness ~ s(Morph_PC2)+s(Morph_PC4),
#                                random= ~(1|Lake), # package gamm4
#                                data=PEAK_PCA)
